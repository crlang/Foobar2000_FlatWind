...
oInputbox = function (w, h, default_text, empty_text, textcolor, backcolor, bordercolor, backselectioncolor, func, parentObjectName) {
	this.font = gdi.Font(g_fname, g_fsize, g_fstyle);
	this.font_italic = gdi.Font(g_fname, g_fsize, g_fstyle);
	this.w = w;
	this.h = h;
	...
	this.setSize = function (w, h, font_size) {
		...
		this.font_italic = gdi.Font("arial", font_size, 0);
	};
	
	this.setfont = function (g_fname, g_fsize, g_fstyle) {
		this.font = this.font_italic = gdi.Font(g_fname, g_fsize, g_fstyle);
	};
	...
	this.show_context_menu = function (x, y) {
		...
		_menu.AppendMenuItem(this.select ? MF_STRING : MF_GRAYED | MF_DISABLED, 1, "¸´ÖÆ");
		_menu.AppendMenuItem(this.select ? MF_STRING : MF_GRAYED | MF_DISABLED, 2, "¼ôÇÐ");
		_menu.AppendMenuSeparator();
		_menu.AppendMenuItem(cInputbox.clipboard ? MF_STRING : MF_GRAYED | MF_DISABLED, 3, "Õ³Ìù");
		if (utils.IsKeyPressed(VK_SHIFT)) {
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 20, "ÊôÐÔ");
			_menu.AppendMenuItem(MF_STRING, 21, "ÅäÖÃ...");
		}
